package com.kata.codewars.rangeExtraction;

import org.junit.jupiter.api.Test;

import static com.kata.codewars.rangeExtraction.RangeExtraction.rangeExtraction;
import static org.junit.jupiter.api.Assertions.*;

class RangeExtractionTest {

    @Test
    void test_BasicTests() {
        assertEquals("-6,4", rangeExtraction(new int[] {-6, 4}));

        assertEquals("-6,4,3", rangeExtraction(new int[] {-6, 4 , 3}));

        assertEquals("-6,2-4", rangeExtraction(new int[] {-6, 2, 3,4}));

        assertEquals("-6--4,2-4", rangeExtraction(new int[] {-6, -5, -4, 2, 3,4}));

        assertEquals("-6,-3-1,3-5,7-11,14,15,17-20", rangeExtraction(new int[] {-6,-3,-2,-1,0,1,3,4,5,7,8,9,10,11,14,15,17,18,19,20}));

        assertEquals("-3--1,2,10,15,16,18-20", rangeExtraction(new int[] {-3,-2,-1,2,10,15,16,18,19,20}));
    }
}